...
function getOAuth2Client(): OAuth2Client {
    ...
    // Set the refresh token (you'll get this from the initial auth flow)
...
// Get calendar client with OAuth2
...
export async function createCalendarEvent(params: CreateEventParams): Promise<{
    ...
    try {
        ...
        const response = await calendar.events.insert({
            calendarId: 'primary', // Use 'primary' for OAuth2 user's calendar
        ...
        if (error.code === 401 || error.message?.includes('invalid_grant')) {
            return {
                success: false,
                message: 'Authentication expired. Please re-authenticate your Google account.',
            };
        }
...
export async function updateCalendarEvent(
    ...
    try {
        ...
        if (params.attendeeEmail) {
            updateData.attendees = [{ email: params.attendeeEmail }];
        }
