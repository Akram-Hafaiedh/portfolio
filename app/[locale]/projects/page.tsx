...
export default function EnhancedProjectsPage() {
    ...
    const { query } = useKBar();
    const projects = getProjects(locale as any);
                    ...
                    <div className="max-w-4xl mx-auto text-center">
                        <div className="space-y-8">
                            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-600 dark:text-blue-400 text-sm font-medium mb-6 animate-fade-in-up">
                                ...
                                {projects.length} {t('hero.projectsCount')}
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-xl text-slate-600 dark:text-slate-400 max-w-3xl mx-auto leading-relaxed animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                                    {t('hero.subtitle')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-600 dark:from-blue-400 dark:to-cyan-400 font-semibold">{t('hero.subtitleHighlight1')}</span> {t('hero.subtitleMiddle')}{' '}
                                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 dark:from-purple-400 dark:to-pink-400 font-semibold">{t('hero.subtitleHighlight2')}</span>
                                    {' '}{t('hero.subtitleEnd')}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 rounded-[2.5rem] p-8 md:p-12 overflow-hidden relative group">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10">
                            <div className="space-y-4">
                                ...
                                <h4 className="text-sm font-black uppercase tracking-widest text-slate-500">{t('stats.completed')}</h4>
                                <div className="text-4xl font-black text-slate-900 dark:text-white">{completedCount}</div>
                                <p className="text-sm text-slate-500 leading-relaxed">{t('stats.completedDesc')}</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <h4 className="text-sm font-black uppercase tracking-widest text-slate-500">{t('stats.inProgress')}</h4>
                                <div className="text-4xl font-black text-slate-900 dark:text-white">{inProgressCount}</div>
                                <p className="text-sm text-slate-500 leading-relaxed">{t('stats.inProgressDesc')}</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <h4 className="text-sm font-black uppercase tracking-widest text-slate-500">{t('stats.technologies')}</h4>
                                <div className="text-4xl font-black text-slate-900 dark:text-white">{totalTech}+</div>
                                <p className="text-sm text-slate-500 leading-relaxed">{t('stats.technologiesDesc')}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="py-20">
                    {filteredProjects.length > 0 ? (
                        ...
                        <div className="text-center py-32">
                            ...
                            <h3 className="text-3xl font-bold text-slate-900 dark:text-white mb-3">{t('results.noProjects')}</h3>
                            <button
